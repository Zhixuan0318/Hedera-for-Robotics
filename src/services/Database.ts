...
export default class Firebase {
    ...
    async getAllOrders(address: string): Promise<Order[]> {
        ...
        const snapshot = await get(ref(this.database, `/hedera/orders/${address.toLowerCase()}`));
    ...
    async getOrder(address: string, orderId: string): Promise<Order> {
        ...
        const snapshot = await get(
            ref(this.database, `/hedera/orders/${address.toLowerCase()}/${orderId}`)
        );
        return snapshot.val();
    }
    ...
    async updateOrder(address: string, data: Order) {
        // if (!this.isAuthed) await this.authDatabase();
        const updates: any = {};
        updates[`/hedera/orders/${address.toLowerCase()}/${data.orderId}`] = data;
    ...
    async completeOrder(address: string, orderId: string, uncompletedOrder: Order) {
        ...
        updates[`/hedera/orders/${address.toLowerCase()}/${orderId}`] = uncompletedOrder;
        await update(ref(this.database), updates);
    }
    ...
    async getOffChainStock(productId: number): Promise<OffChainStock> {
        const snapshot = await get(ref(this.database, `/hedera/stocks/${productId}`));
        return snapshot.val();
    }

    async getAllOffChainStocks(): Promise<OffChainStock[]> {
        const snapshot = await get(ref(this.database, `/hedera/stocks`));
    ...
    async replenish(productId: number, amount: number) {
        // if (!this.isAuthed) await this.authDatabase();
        const snapshot = await get(ref(this.database, `/hedera/stocks/${productId}/stock`));
        const updates: any = {};
        updates[`/hedera/stocks/${productId}/stock`] = snapshot.val() + amount;
        await update(ref(this.database), updates);
    }
    ...
    async addOnHold(productId: number) {
        ...
        updates[`/hedera/stocks/${productId}`] = data;
        await update(ref(this.database), updates);
    }
