...
export default function Approval({ order }: { order: Order | undefined }) {
    ...
    const provider = useRef(new JsonRpcProvider(process.env.PROVIDER, hederaTestnet.id));
    ...
    const updateApprovals = async (copy: string[]) => {
        ...
        if (order.hashes.length > 4 && !copy[0])
            copy[0] = await setApprovalAddress(order.hashes[3].slice(0, 66));
        if (order.hashes.length > 6 && !copy[1])
            copy[1] = await setApprovalAddress(order.hashes[5].slice(0, 66));
        if (order.hashes.length == 8 && order.hashes[7] != 'empty' && !copy[2])
            copy[2] = await setApprovalAddress(order.hashes[7].slice(0, 66));
        return copy;
    };
    ...
    const setApprovalAddress = useCallback(async (hash: string): Promise<string> => {
        ...
        return transaction?.logs.length
            ? '0x' + transaction.logs[2].topics[1].slice(26)
            : addresses.owner;
    }, []);
